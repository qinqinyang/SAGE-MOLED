...
%    voxel.
res_csvd.map=zeros(options.nR,options.nC);
if options.deconv.SVD.residual
    res_csvd.residual=zeros(options.nR,options.nC,nTpad);
end
...
for r=1:options.nR
    ...
    for c=1:options.nC
            if mask(r,c)
                ...
                vettConc(1:options.nT)=reshape(conc(r,c,:),options.nT,1);
                vettRes=(1/options.tr)*Ginv*vettConc;
                
                res_csvd.map(r,c)=max(abs(vettRes));
                if options.deconv.cSVD.residual
                    res_csvd.residual(r,c,:)=vettRes;
                end
            end
    end
end
